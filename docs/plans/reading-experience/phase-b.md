...
_Coordination note: A4 (Phase A) appends `--hl-mix`/`--hl-blend` to the same `:root` and theme blocks. Either order works — anchor on the quoted text, not line numbers._
...
_(Skip this step if E11 already applied — it includes `text-wrap: balance`.)_

...
2. `apps/web/src/routes/read/[id]/+page.svelte:545-546` — Escape closes popovers first (the global key layer routes Escape to `controller.back`). _Coordination: if A5 (capture-phase Escape handler) is already applied, add `menuOpen` handling to that handler instead of this step, and skip this step._
...
1. `apps/web/src/lib/typography.ts:213-220` — add `THEME_TEXT` and the utility functions after `THEME_BG`.
   ...
   _(Coordination: if E6 has landed, `THEME_TEXT.black` should be `#cfcdc8` to match.)_
